...
module.exports = async function handleKoiBINCommand(socket, chat, messageText) {
  ...
  // Ekstrak kode koi dan nilai BIN baru dari pesan
  ...
  if (!kodeIkan || !newOpenBid || isNaN(newOpenBid) || parseFloat(newOpenBid) <= 0) {
    await socket.sendMessage(
      chat.key.remoteJid,
      {
        text: "⚠️ Format tidak benar. Gunakan format: koi-BIN #kode_koi#nilai_BIN (contoh: 500 untuk 500 ribu rupiah)",
      },
      { quoted: chat }
    );
    return;
  ...
  db.query(
    ...
    async (err, results) => {
      ...
      // Update BIN ikan koi
      db.query(
        "UPDATE koi SET buy_it_now = ? WHERE kode_ikan = ? AND seller_id = ?",
        [parseFloat(newOpenBid), kodeIkan, sellerId],
        async (err, results) => {
          if (err) {
            console.error("Error updating koi BIN:", err);
            await socket.sendMessage(
              chat.key.remoteJid,
              { text: "🛑 Terjadi kesalahan saat mengupdate nilai BIN koi. Silakan coba lagi nanti." },
              { quoted: chat }
            );
            return;
          }
          ...
          if (results.affectedRows > 0) {
            await socket.sendMessage(
              chat.key.remoteJid,
              { text: `✅ Nilai BIN ikan dengan kode *${kodeIkan}* berhasil diubah menjadi *${newOpenBid} K*.` },
              { quoted: chat }
            );
